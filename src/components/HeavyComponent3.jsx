...
function HeavyComponent3() {
  ...
  return (
    <div style={{ padding: '2rem', background: '#f0fff0', borderRadius: '8px' }}>
      ...
      <div style={{ marginTop: '1rem', display: 'flex', gap: '0.5rem' }}>
        <button onClick={() => setFilter('all')} style={{ background: filter === 'all' ? '#667eea' : 'white', color: filter === 'all' ? 'white' : 'black' }}>
          All
        </button>
        <button onClick={() => setFilter('A')} style={{ background: filter === 'A' ? '#667eea' : 'white', color: filter === 'A' ? 'white' : 'black' }}>
          Category A
        </button>
        <button onClick={() => setFilter('B')} style={{ background: filter === 'B' ? '#667eea' : 'white', color: filter === 'B' ? 'white' : 'black' }}>
          Category B
        </button>
        <button onClick={() => setFilter('C')} style={{ background: filter === 'C' ? '#667eea' : 'white', color: filter === 'C' ? 'white' : 'black' }}>
          Category C
        </button>
      </div>

      <div style={{ marginTop: '1rem', background: 'white', padding: '1rem', borderRadius: '4px' }}>
        <h4>Statistics:</h4>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', textAlign: 'center' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>{stats.avg}</div>
          </div>
          <div>
            <div style={{ fontSize: '0.875rem', color: '#666' }}>Min</div>
            <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>{stats.min}</div>
          </div>
          <div>
            <div style={{ fontSize: '0.875rem', color: '#666' }}>Max</div>
            <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>{stats.max}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ marginTop: '1rem', maxHeight: '200px', overflow: 'auto', background: 'white', padding: '1rem', borderRadius: '4px' }}>
        ...
        {filteredData.slice(0, 30).map(point => (
          <div key={point.day} style={{ padding: '0.25rem', borderBottom: '1px solid #eee', display: 'flex', justifyContent: 'space-between' }}>
